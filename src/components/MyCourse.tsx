...
type ProgressChartProps = {
  percentage: number; // Pass the progress percentage as a prop
};
...
const ProgressChart: React.FC<ProgressChartProps> = ({ percentage }) => {
  ...
  return (
    <RadialBarChart
      width={50} // Set the chart width to 50px
      height={50} // Set the chart height to 50px
      data={chartData}
      startAngle={0} // Adjust start angle
      endAngle={percentage * 3.6} // Convert percentage to degrees
      innerRadius={20} // Adjust inner radius
      outerRadius={30} // Adjust outer radius
    >
      <PolarGrid
        gridType="circle"
        radialLines={false}
        ...
        polarRadius={[22, 18]} // Adjust radius for 50px dimensions
      />
              ...
              return (
                ...
                >
                  <tspan
                    x={viewBox.cx}
                    y={viewBox.cy}
                    className="text-foreground text-[10px] font-bold dark:fill-white"
                  >
            ...
            myCourse?.map((item, index) => (
              <div key={index} className="flex justify-between last:mb-[10px]">
                <div className="flex items-start">
                  ...
                  <div className="pl-[10px]">
                    ...
                    <div className="flex gap-[5px] flex-wrap">
                      <p className="flex gap-[5px] items-center text-[14px] text-black opacity-60 dark:text-white">
                ...
                <div className="flex items-center gap-[10px] pl-[10px]">
                  <p className="text-[14px] text-black opacity-60 dark:text-white">{item?.tag}</p>
                  <ChartContainer
                    config={chartConfig}
                    className="mx-auto aspect-square w-[50px] h-[50px]"
                  >
                    <ProgressChart percentage={item?.progress} />
                  </ChartContainer>
